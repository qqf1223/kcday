    ...
    public function response()
    {
        try{
            ...
            if($type == 'json'){
                ...
                //判断结果
                if(isset($handleResult['status']) && !$handleResult['status']) return $this->responseError($handleResult['msg']);
            ...
            if($handleResult['status']){
                //成功状态
                //是否指向跳转地址
                if(empty($urlOrIsOriginalOutput)) return view()->with('prompt', ['status' => 1, 'msg' => $handleResult['msg']]);
