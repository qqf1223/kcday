	...
	if(options){
		...
		$.ajax(opt).done(function(obj){
			...
			if(fail){
				...
				qiao.bs.msg({msg:'数据传输失败，请重试！',type:'danger'});
				return false;
			}
		});
	}
...
qiao.bs.msgstr = function(msg, type, position){
	return '<div class="alert alert-'+type+' alert-dismissible bsalert" role="alert" style="display:none;position:fixed;' + position + ':0;left:0;width:100%;z-index:2001;margin:0;text-align:center;" id="bsalert"><button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>'+msg+'</div>';
};
qiao.bs.msg = function(options,type){
	...
	if(tree){
		...
		if(opt.edit) {
			if(tree.id == 0){
				...
				res +=
					'&nbsp;&nbsp;<span class="label label-primary bstreeadd" style="display:none;">添加</span>' +
					'&nbsp;&nbsp;<span class="label label-primary bstreeedit" style="display:none;">修改</span>' +
					'&nbsp;&nbsp;<span class="label label-danger  bstreedel" style="display:none;">删除</span>';
			}
		}
			res += '</a>';
...
qiao.bs.tree.init = function(){
	...
	qiao.on('#treeul li a', 'mouseover', function(){
		$(this).children(".label").css('display','');
	});
	qiao.on('#treeul li a', 'mouseout', function(){
...
qiao.bs.tree.addp = function(){
	$('#bsmodal').remove();
	qiao.bs.dialog({
		url 	: qiao.bs.tree.url + '/add/category_id/' + $(this).parent().qdata().id,
		title 	: '添加类别',
		okbtn 	: '保存'
	}, qiao.bs.tree.add);
};
...
qiao.bs.tree.del = function(){
	var category_id = $(this).parent().qdata().id;
	qiao.bs.confirm({
		msg  :'确认要删除此产品类别吗?',
		small: true
	},function(){
		qiao.ajax({
			url:qiao.bs.tree.url + '/del',
...
qiao.bs.tree.editp = function(){
	qiao.bs.dialog({
		url 	: qiao.bs.tree.url + '/edit/category_id/' + $(this).parent().qdata().id,
		title 	: '修改类别',
		okbtn 	: '保存'
	}, qiao.bs.tree.edit);
};
...
qiao.crud.list = function(data){
	...
	*/
	$('#cruddiv').bstree({url:'/category', edit:true});
};
...
qiao.crud.savep = function(title, id){
	var url = id ? (qiao.crud.url + '/edit/id/' + id) : (qiao.crud.url + '/edit');
